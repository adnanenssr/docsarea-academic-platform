...
function CallUserViews() {
  ...
  const getViewsByWeek = async (beginning, ending) => {
    try {
      const response = await fetch(`http://localhost:8082/get/views/user?start=${beginning}&end=${ending}`, {
        method: "GET",
        credentials: "include"
      })
  ...
  const getViewsByFiles = async (beginning, ending) => {
    try {
      const response = await fetch(`http://localhost:8082/get/views/files?start=${beginning}&end=${ending}&page=0`, {
        method: "GET",
        credentials: "include"
      })
