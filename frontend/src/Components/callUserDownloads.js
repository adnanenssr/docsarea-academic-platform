...
function CallUserDownloads() {
  ...
  const getDownloadsByWeek = async (beginning, ending) => {
    try {
      const response = await fetch(`http://localhost:8082/get/downloads/user?start=${beginning}&end=${ending}`, {
        method: "GET",
        credentials: "include"
      });
  ...
  const getDownloadsByFiles = async (beginning, ending) => {
    try {
      const response = await fetch(`http://localhost:8082/get/downloads/files?start=${beginning}&end=${ending}&page=0`, {
        method: "GET",
        credentials: "include"
      });
...
export default CallUserDownloads